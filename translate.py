...
def get_all_translations(rna_sequence, genetic_code):
    ...
    """
    rna_sequence = rna_sequence.upper()
    nucleotides = len(rna_sequence)
    index = nucleotides - 3
    if len(rna_sequence) < 3:
        return ""
    amino_acid_seq_list = []
    for nucleotide_index in range(nucleotides +1):
        codon = rna_sequence[nucleotide_index: nucleotide_index +3]
        if codon == "AUG":
            aa_seq = translate_sequence(rna_sequence = rna_sequence[nucleotide_index:],
            genetic_code = genetic_code)
            if aa_seq: amino_acid_seq_list.append(aa_seq)
...
def get_longest_peptide(rna_sequence, genetic_code):
    ...
    all_trans = get_all_translations(rna_sequence = rev_comp, genetic_code = genetic_code)
    peptides += all_trans
    if not peptides:
        return ""
    if len(peptides) < 2:
    ...
    for peptide, aa_seq in enumerate(peptides):
        if len(aa_seq) > most_nucleotides:
            longest_peptide = peptide
            most_nucleotides = len(aa_seq)
